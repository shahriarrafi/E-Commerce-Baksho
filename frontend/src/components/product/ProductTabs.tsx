...
export default function ProductTabs({ description, specifications, shippingInfo }: ProductTabsProps) {
  ...
  return (
    <div className="w-full mt-8 md:mt-12 mb-12 md:mb-20">
      {/* Tab Headers */}
      <div className="flex border-b border-brand-cream/50 relative overflow-x-auto scrollbar-hide -mx-6 px-6 sm:mx-0 sm:px-0 no-scrollbar font-hind">
        {TABS.map((tab) => (
          <button
            key={tab}
      ...
      <div className="py-10 px-4 min-h-[300px]">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="prose prose-brand-navy max-w-none font-hind"
          >
            {activeTab === "বিস্তারিত" && (
              <div className="space-y-6">
                <h3 className="text-2xl font-serif text-brand-navy">পণ্যের পেছনের গল্প</h3>
            ...
            {activeTab === "শিপিং তথ্য" && (
              <div className="flex flex-col gap-8 md:flex-row md:items-center">
                <div className="flex-1 space-y-4">
                  <h3 className="text-2xl font-serif text-brand-navy">গ্লোবাল কারুশিল্প, লোকাল ডেলিভারি</h3>
                  <p className="text-brand-navy/70 leading-relaxed text-lg font-light">
                    {shippingInfo}
                  </p>
                </div>
